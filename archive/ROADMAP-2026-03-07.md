> **HISTORICAL DOCUMENT — superseded, not current.**
>
> This is `ROADMAP.md` exactly as it stood at its last edit, **2026-03-07**. It is
> kept because the way this company worked in March is part of the public record,
> not because any of it is still true. **Nothing below describes how The Website
> operates today.**
>
> Specifically, and verifiably wrong as of 2026-07-30:
>
> - **"Course Instructor" was never a role in the current model.** The roster is
>   CEO, course-content, seo-growth, product-manager, engineer, code-reviewer,
>   content-reviewer — see [`TEAM.md`](../TEAM.md).
> - **The `$80,000/month` goal is not a target this business holds.** It appears
>   in no source of truth. [`COURSE_FACTS.md`](../COURSE_FACTS.md) is
>   authoritative for every stated fact, and it does not contain it.
> - **The daily-Twitter and HN-monitoring tasks below are not running, and would
>   not be permitted to.** External community posting is reserved for the human
>   owner ([`OPERATIONS.md`](../OPERATIONS.md) §2/§4). The "Automated via
>   scheduled task" note was already inaccurate; exactly one automation exists
>   (`ceo-coordinator`).
> - **`twitter_launch_thread.md`, referenced below, does not exist** in this repo.
> - **The task statuses are frozen at 2026-03-07.** The live backlog is GitHub
>   Issues; the live activity record is [`/activity`](https://www.thewebsite.app/activity).
>
> For how the company actually runs, read [`OPERATIONS.md`](../OPERATIONS.md)
> (binding) and [`TEAM.md`](../TEAM.md) (roster).
